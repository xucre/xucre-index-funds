...
const OrganizationInvoiceTable = ({organizationId}: {organizationId: string}) => {
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      ...
      const invoiceIds = await getAllOrganizationInvoices(organizationId);
      const invoicePromises = invoiceIds.map(async (invoiceId: string) => {
        const invoiceData = await getInvoiceDetails(organizationId, invoiceId);
      ...
      <DataGrid
        ...
        columns={[
          ...
          },
          {
            field: 'action',
            headerName: 'Action',
            flex: 1, headerClassName: 'primaryBackground--header',
            renderCell: (params) => (
              <IconButton onClick={() => router.push(`/organizations/id/${organizationId}/invoice/${params.row.id}`)}>
                <ArrowForwardIcon />
              </IconButton>
            ),
          },
...
export default OrganizationInvoiceTable;