...
export const getWalletHistory = async (wallet: string, chainName: string, hardReset: boolean = false) => {
  try {
    const response = await superagent.get(`${BASEURL}history?chainName=${chainName}&wallet=${wallet}${hardReset ? `&hard=true`:''}`).withCredentials();
    return response.body;
  } catch (error) {
    return null;
  }
...
export const getWalletTransactions = async (wallet: string, chainName: any) => {
  try {
    const response = await superagent.get(`${BASEURL}transactions?chainName=${chainName}&wallet=${wallet.toLowerCase()}&hard=false`).withCredentials();
    return response.body;
  } catch (error) {
    return null;
  }
